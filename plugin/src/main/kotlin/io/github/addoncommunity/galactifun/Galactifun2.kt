...
object Galactifun2 : AbstractAddon() {
    ...
    override suspend fun onEnableAsync() {
        ...
        // Classic Galactifun (Drake fork plugin name: Galactifun-drake) can run alongside GF2 if worlds/recipes are kept disjoint.
        val legacyGalactifun = sequenceOf("Galactifun-drake", "Galactifun")
            .filter { Bukkit.getPluginManager().isPluginEnabled(it) }
            .toList()
        if (legacyGalactifun.isNotEmpty()) {
            logger.warning(
                "Galactifun2 also sees plugin(s): ${legacyGalactifun.joinToString()}. " +
                    "Use /gf2 and /galactifun2 for GF2 (classic keeps /gf). Watch for overlapping Slimefun item IDs and world configs."
            )
        }
