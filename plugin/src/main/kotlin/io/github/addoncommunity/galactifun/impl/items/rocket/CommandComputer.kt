...
class CommandComputer(
    ...
    private fun rescanRocket(pos: BlockPosition) {
        ...
        blocks.processSlimefunBlocks<Seat, Unit> { Seat.setRocket(it.location, pos) }
        val engines = blocks.processSlimefunBlocks<RocketEngine, Pair<Pair<RocketEngine, BlockPosition>, MutableSet<BlockPosition>>> { b ->
            val fuel = b.blockPosition.floodSearch { it.checkBlock<FuelTank>() != null }.found.toMutableSet()
            fuel.remove(b.blockPosition)
            (this to b.blockPosition) to fuel
        }.toMap()
